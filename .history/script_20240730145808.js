...
async function saveTaskText(target, currentTaskId, task) {
  console.log(target.value, currentTaskId, task.id)
  const newText = target.value;
  const task333 = tasks.find(task => task.id === currentTaskId);
  

  if (task333 && task333.text !== newText) {
    console.log(888)
      task333.text = newText;
